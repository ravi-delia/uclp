...
(defun compile-lenprefix (opts expr) ;; Grouped in with capture because it manipulates the stack
  (with-gensyms ($matched $capped $count)
    (destructuring-bind (n-pat r-pat) (rest expr)
...
(defun compile-integer (opts expr)
  (with-gensyms ($region $reg-len $int $int-len)
    (destructuring-bind (pat &optional radix tag) (rest expr)
      `(with-save (curr)
	 (when ,(compile-expr opts pat)

...
(defparameter *compiled* t)

(defun matches? (pat str start args &optional (expect-caps nil check-caps?))
  (multiple-value-bind (success? caps)
      (apply #'uclp:match
	     (if *compiled* (compile-peg pat) pat)
	     str start args)
    (and success?
	 (if check-caps?
...
(defmacro asm-test (name &body body)
  `(progn
    (test ,name ,@body)
    (test ,(symbolicate 'ASM- name)
       (let ((*compiled* nil)) ,@body))))
...
(defmacro isnt-match (pat str &key (start 0) (args nil))
  `(fiveam:is-false (matches? ,pat ,str ,start ,args)))
